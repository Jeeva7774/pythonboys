...
const getOpenAIKey = () => {
  ...
  const firebaseKey = functions.config().openai?.key;
  if (firebaseKey) {
 ...
 */
export const healthCheck = regionalFunctions.https.onCall(async (data: any, context: functions.https.CallableContext) => {
 ...
 */
export const aiChatHandler = regionalFunctions.https.onCall(async (data, context) => {
 ...
 */
export const onFocusSessionCreate = regionalFunctions.firestore
  ...
  .onCreate(async (snap, context) => {
    ...
    try {
      ...
      const lastSessionDate = userData.lastSessionDate?.toDate?.()?.toDateString();
 ...
 */
export const onPostCreate = regionalFunctions.firestore
 ...
 */
export const weeklyLeaderboardRollup = regionalFunctions.pubsub
    ...
    try {
      ...
      for (const communityDoc of communitiesSnapshot.docs) {
        ...
        await db.collection('leaderboards').doc(leaderboardId).set({
          ...
          entries: leaderboardEntries.slice(0, 50), // Top 50
          createdAt: admin.firestore.FieldValue.serverTimestamp(),
        });
 ...
 */
export const sendNotification = regionalFunctions.https.onCall(async (data, context) => {
  if (!context.auth) {
    throw new functions.https.HttpsError('unauthenticated', 'User must be authenticated');
  }
...
async function checkAndAwardAchievements(userId: string, stats: {
  ...
  const userRef = db.collection('users').doc(userId);
  const userDoc = await userRef.get();
  const earnedAchievements = userDoc.data()?.achievements || [];
...
async function updateCommunityLeaderboards(userId: string, pointsEarned: number) {
  try {
    const userDoc = await db.collection('users').doc(userId).get();
